...
class BidirectionalSearchAgent(Agent):
    ...
    def __init__(self, fn='depthFirstSearch', prob='BidirectionalPositionSearchProblem', heuristic='nullHeuristic',
                 backwardsHeuristic='nullHeuristic'):
        # Warning: some advanced Python magic is employed below to find the right functions and problems
        ...
        if 'heuristic' not in func.__code__.co_varnames:
            ...
            print('[BidirectionalSearchAgent] using function %s, heuristic %s and backwardsHeuristic %s' % (
            fn, heuristic, backwardsHeuristic))
            # Note: this bit of Python trickery combines the search algorithm and the heuristic
...
class BidirectionalFoodSearchAgent(BidirectionalSearchAgent):
    ...
    def __init__(self, fn='depthFirstSearch', prob='BidirectionalFoodSearchProblem', heuristic='nullHeuristic',
                 backwardsHeuristic='nullHeuristic'):
...
class BidirectionalFoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        # Goal states: foods lists are empty, and pacman stays at the position of last-eaten food
        self.goal_state_list = []
        for food in self.food_list:
            self.goal_state_list.append((food, []))
        # Indicator of goal state
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            current_position = x, y = state[0]  # Coordinate of current state
            change_position = dx, dy = Actions.directionToVector(action)  # Change in coordinate
            next_position = next_x, next_y = int(x + dx), int(y + dy)  # Coordinate of successor

            # If the successor is valid, i.e., not wall
            if not self.walls[next_x][next_y]:
                # Create a food list for successor
                next_food_list = state[1].copy()

                # If there is food at next position, remove that food from food list
                if next_position in next_food_list:
                    next_food_list.remove(next_position)

                """
                # An abandoned approach, which works but is longer in time
                next_food_grid = Grid(self.foodGrid.width, self.foodGrid.height)
                for coordinate in next_food_list:
                    next_food_grid[coordinate[0]][coordinate[1]] = True
                if next_food_grid[next_x][next_y]:
                    next_food_grid[next_x][next_y] = False
                next_food_list = next_food_grid.asList()
                """

                # Create a state for successor, and use cost = 1
                next_state = (next_position, next_food_list)
                cost = 1
                # Create successor and add successor to successors list
                successor = (next_state, action, cost)
    ...
    def getBackwardsSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            current_position = x, y = state[0]  # Coordinate of current state
            change_position = dx, dy = Actions.directionToVector(action)  # Change in coordinate
            prev_position = prev_x, prev_y = int(x + dx), int(y + dy)  # Coordinate of successor

            # If the successor is valid, i.e., not wall
            if not self.walls[prev_x][prev_y]:
                # Create a food list for successor
                prev_food_list = state[1].copy()

                # If there should be a food in current position but pacman has already eaten it, then we should return it
                # Create a new grid and put foods in successor's food list into their position
                prev_food_grid = Grid(self.foodGrid.width, self.foodGrid.height)
                for coordinate in prev_food_list:
                    prev_food_grid[coordinate[0]][coordinate[1]] = True
                # Put the food back to current position if there should be a food but pacman has already eaten it
                if self.foodGrid[x][y] and not prev_food_grid[x][y]:
                    prev_food_grid[x][y] = True
                # Recreate the food list for successor
                prev_food_list = prev_food_grid.asList()

                """
                # An abandoned approach, which works but won't produce optimal solution (always 1 more than solution length)
                if current_position in self.food_list and current_position not in prev_food_list:
                    prev_food_list.append(current_position)
                """

                # Create a state for successor, and use cost = 1, and reverse the action
                prev_state = (prev_position, prev_food_list)
                rev_action = Actions.reverseDirection(action)
                cost = 1
                # Create successor and add successor to successors list
                successor = (prev_state, rev_action, cost)
                successors.append(successor)
